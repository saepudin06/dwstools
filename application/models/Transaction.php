...

{
    ...
    function runAllTrx($trx, $id){

        // getAllTrx INIT 

        // update trx to Processing
...
z
   ...
   /* function getJsonDataTrx($id, $refId, $code){
         
         $sql = " select 
                    ";

        $parse = oci_parse($this->db->conn_id,$sql);
        oci_bind_by_name($parse,':userid', $userinfo->username, 255);
        oci_execute($parse);
        OCIFetch($parse);

        $cek = OCIResult($parse, 'ID');
