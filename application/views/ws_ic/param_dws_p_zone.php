...
<?php $this->load->view('lov/lov_sap_zone'); ?>
<script>
    $(function() {
        set_grid();
    });
    ...
    function set_grid(){
        ...
        if (is_set_grid != 'true'){
            ...
            jQuery(grid_selector).jqGrid({
                url: '<?php echo WS_JQGRID."ws_ic.param_dws_p_zone_controller/crud"; ?>',
                datatype: "json",
                mtype: "POST",
                colModel: [
                    {label: 'P Zone ID', name: 'p_zone_id', key: true, align: 'center', width: 150, hidden: true, editable: true },
                    {label: 'SAP Code ID', name: 'p_sap_zone_id', align: 'center', width: 150, editable: true, hidden: true,
                    ...
                    {label: 'SAP Code', name: 'sap_code', align: 'center', width: 150, editable: true, editrules: {required: true},
                        edittype: 'custom',
                        editoptions: {
                            "custom_element":function( value  , options) {
                                ...
                                setTimeout( function() {
                                    elm.append(
                                            '<input id="form_sap_code" readonly type="text" class="FormElement form-control jqgrid-required">'+
                                            '<button class="btn btn-success" style="margin-bottom: 2px; margin-left: 2px;" type="button" onclick="modal_lov_sap_zone_show(\'tr_p_sap_zone_id .DataTD #p_sap_zone_id\', \'form_sap_code\')">'+
                            ...
                            "custom_value":function( element, oper, gridval) {
                                ...
                                console.log(gridval);
                                if(oper === 'get') {
                                    return $("#form_sap_code").val();
                                } else if( oper === 'set') {
                                    $("#form_sap_code").val(gridval);
                                    var gridId = this.id;
                                    // give the editor time to set display
                                    setTimeout(function(){
                                        var selectedRowId = $("#"+gridId).jqGrid ('getGridParam', 'selrow');
                                        if(selectedRowId != null) {
                                            var code_display = $("#"+gridId).jqGrid('getCell', selectedRowId, 'sap_code');
                                            $("#form_sap_code").val( code_display );
                                        }
                                    },100);
                                }
                            }
                    ...
                    {label: 'Zone Soki', name: 'zone_soki', align: 'center', width: 150, editable: true, 
                        editoptions:{
                            size: 30,
                            maxlength: 5
                        }
                ...
                editurl: '<?php echo WS_JQGRID."ws_ic.param_dws_p_zone_controller/crud"; ?>',
                caption: "Zone"
